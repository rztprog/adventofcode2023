...
const initialSeeds = text.match(/seeds:\s(.*)/)[1].split(/\s+/).map(Number)
const lines = text.split('\n');
const sections = [];
let locationNumber = initialSeeds[0];
let currentSection = [];
...
for (let index = 0; index < initialSeeds.length; index+=2) {
    for (let seed = initialSeeds[index]; seed <= (initialSeeds[index] + initialSeeds[index + 1] - 1); seed++) {
